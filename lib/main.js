...
function dispatchBy( fcns, types, data, nargs, nin, nout ) {
	...
	if ( isFunction( fcns ) ) {
		...
		throw new TypeError( format( '1fZ5d', fcns ) );
	}
	if ( !isCollection( types ) ) {
		throw new TypeError( format( '1fZ2y', types ) );
	}
	if ( !isCollection( data ) && data !== null ) {
		throw new TypeError( format( '1fZ5e', data ) );
	}
	if ( !isPositiveInteger( nargs ) ) {
		throw new TypeError( format( '1fZ5f', nargs ) );
	}
	if ( !isNonNegativeInteger( nin ) ) {
		throw new TypeError( format( '1fZ5X', nin ) );
	}
	if ( !isNonNegativeInteger( nout ) ) {
		throw new TypeError( format( '1fZ5g', nout ) );
	}
	narrays = nin + nout;
	if ( narrays === 0 ) {
		throw new Error( format('1fZ1M') );
	}
	if ( fcn ) {
		nfcns = types.length / narrays;
		if ( !isInteger( nfcns ) ) {
			throw new Error( format('1fZ1N') );
		}
	} else {
		nfcns = fcns.length;
	...
	function dispatcher() {
		...
		if ( argc !== nargs ) {
			...
			if ( argc === nargs+1 ) {
				...
				throw new Error( format('1fZ0m') );
			}
		}
		N = arguments[ 0 ];
		if ( !isInteger( N ) ) {
			throw new TypeError( format( '1fZ7e', N ) );
		}
		shape = [ N ];

		clbk = arguments[ argc-1 ];
		if ( !isFunction( clbk ) ) {
			throw new TypeError( format( '1fZ2b', clbk ) );
		}
		...
		for ( i = 3; i < nargs; i += strideArgs ) {
			...
			if ( !isInteger( v ) ) {
				if ( i < iout ) {
					throw new TypeError( format( '1fZAH', v ) );
				} else {
					throw new TypeError( format( '1fZAI', v ) );
				}
			}
			strides.push( v );
		}
		if ( hasOffsets ) {
			...
			for ( i = 4; i < nargs; i += strideArgs ) {
				...
				if ( !isNonNegativeInteger( v ) ) {
					if ( i < iout ) {
						throw new TypeError( format( '1fZAG', v ) );
					} else {
						throw new TypeError( format( '1fZAE', v ) );
					}
				}
				offsets.push( v );
			}
		...
		for ( i = 2; i < nargs; i += strideArgs ) {
			...
			if ( !isCollection( v ) ) {
				if ( i < iout ) {
					throw new TypeError( format( '1fZDg', v ) );
				} else {
					throw new TypeError( format( '1fZDh', v ) );
				}
			}
			j = (i-2) / strideArgs;
			if ( hasOffsets ) {
				...
				if ( N > 0 && (idx < 0 || idx >= v.length) ) {
					if ( i < iout ) {
						throw new RangeError( format('1fZ1T') );
					} else {
						throw new RangeError( format('1fZ1U') );
					}
				}
		...
		if ( idx < 0 ) {
			throw new TypeError( format('1fZ1L') );
		}
